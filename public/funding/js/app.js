...
const radioBtnBank = document.querySelector(".radio-btn-bank");
...
// for (let i = 0; i <= radioBtns.length - 1; i++) {
//   radioBtnBank[i].addEventListener("click", () => {
//     for (let j = 0; j < openSection.length; j++) {
//       openSection[i].style.display = "none";
//     }
//     if (!openSection[j]) return;
//     openSection[j].style.display = "none" ? "flex" : "";
//   });
// }

radioBtnBank.addEventListener("click", () => {
  openSection[0].style.display = "none";

  if (!openSection[1]) return;
  openSection[1].style.display = "none" ? "flex" : "";
});
